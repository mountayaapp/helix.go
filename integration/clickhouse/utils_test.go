...
func TestNormalizeErrorMessage(t *testing.T) {
	testcases := []struct {
	...
	}{
		{
			...
			input:    errors.New("failed to connect to `host=localhost user=default database=default`"),
			expected: "Failed to connect to `host=localhost user=default database=default`",
		},
		{
